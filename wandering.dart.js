...
Isolate.$defineClass("Pen", "Object", ["path?"], {
 Pen$1: function(startPosition) {
  this.path = $.Path$0();
  this.path.createStartSegment$1(startPosition);
 }
});

Isolate.$defineClass("Path", "Object", ["segments?"], {
 lastLine$1: function(segment) {
  if (!$.eqNullB(segment)) {
    if ($.isEmpty(segment.get$lines()) !== true) {
      var result = $.last(segment.get$lines());
    } else {
      var previous = this.previousSegment$1(segment);
      if (!$.eqNullB(previous)) {
        result = this.lastLine$1(previous);
      } else {
        result = (void 0);
      }
    }
 ...
 previousSegment$1: function(segment) {
  if (!$.eqNullB(segment)) {
    var index = $.indexOf$1(this.segments, segment);
    if ($.gtB(index, 0)) {
      var result = $.index(this.segments, $.sub(index, 1));
    } else {
      result = (void 0);
    }
  } else {
 ...
 lastSegment$0: function() {
  if ($.isEmpty(this.segments) !== true) {
    var result = $.last(this.segments);
  } else {
    result = (void 0);
  }
  return result;
 },
 createStartSegment$1: function(startPosition) {
  var startSegment = $.Segment$1(false);
  $.add$1(startSegment.lines, $.Line$2(startPosition, startPosition));
  $.add$1(this.segments, startSegment);
 },
 Path$0: function() {
  var t1 = $.List((void 0));
...
Isolate.$defineClass("Segment", "Object", ["lines?", "draw?"], {
 last$0: function() {
  if ($.isEmpty(this.lines.get$size()) !== true) {
    var result = $.last(this.lines);
  } else {
    result = (void 0);
  }
  return result;
 },
 Segment$1: function(draw) {
  var t1 = $.List((void 0));
...
Isolate.$defineClass("Line", "Object", ["endPoint?", "beginPoint?"], {
});
...
Isolate.$defineClass("Closure2", "Closure10", ["box_0"], {
 $call$0: function() {
...
Isolate.$defineClass("Closure4", "Closure10", ["box_0"], {
 $call$0: function() {
...
Isolate.$defineClass("Closure6", "Closure10", ["this_0"], {
 $call$0: function() {
  return $._ElementRectImpl$1(this.this_0);
 }
});

Isolate.$defineClass("Closure7", "Closure10", [], {
 $call$1: function(e) {
  return $._completeMeasurementFutures();
 }
});

Isolate.$defineClass("Closure8", "Closure10", [], {
 $call$0: function() {
  return $.CTC5;
 }
});
...
$._maybeScheduleMeasurementFrame = function() {
  ...
  if ($._firstMeasurementRequest === true) {
    $.add$1($.window().get$on().get$message(), new $.Closure7());
...
$.Segment$1 = function(draw) {
  var t1 = new $.Segment((void 0), draw);
  t1.Segment$1(draw);
  return t1;
};
...
$.Line$2 = function(beginPoint, endPoint) {
  return new $.Line(endPoint, beginPoint);
};
...
$.main = function() {
  ...
  $.pen = $.Pen$1($.center());
  $.segmentButton = $.document().query$1('#segment');
  $.add$1($.segmentButton.get$on().get$click(), new $.Closure());
  $.document().get$window().setInterval$2($.draw, 10);
};
...
$.$defineNativeClass('HTMLHRElement', ["width?", "size?"], {
});
...
$.$defineNativeClass('HTMLInputElement', ["width?", "value=", "size?", "pattern?", "height?"], {
 get$on: function() {
...
$.$defineNativeClass('HTMLSelectElement', ["value=", "size?", "length="], {
});
...
$.$defineNativeClass('TextTrackCue', ["text!", "size?"], {
 $dom_addEventListener$3: function(type, listener, useCapture) {
  return this.addEventListener(type,$.convertDartClosureToJS(listener, 1),useCapture);
 },
 get$on: function() {
